...
- **Storage**: D1 `migrations/0001_agent_router.sql` (`users`, `gateway_keys`, `providers`, `provider_keys`, `usage_ledger`; legacy schema archived in `migrations/_archive/`) + `migrations/0002_codex_oauth.sql` (OAuth columns on `provider_keys`, `codex_oauth_sessions`). Upstream secrets AES-GCM encrypted via Secrets Store (`AES_ENCRYPTION_KEY_SECRET`, `KeyCrypto`).
...
- **API**: `apps/api` Hono `AgentRouterWorker` (`POST /v1/chat/completions|embeddings|responses`, `GET /v1/models`, `POST /anthropic/v1/messages`, `POST /gemini/v1beta/models/:action` + `/user/providers|gateway-keys|usage|me` (providers include Codex device-code + token-import endpoints) + `/health`, `/docs`); model-prefix routing with `x-provider-id` override (`OPENAI` falls back to `OPENAI_CODEX`).
