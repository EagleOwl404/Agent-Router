...
- **Gateway**: OpenAI-compatible `POST /v1/chat/completions`, `POST /v1/embeddings`, `POST /v1/responses`, `GET /v1/models` + `POST /anthropic/v1/messages` + `POST /gemini/v1beta/models/<model>:generateContent`. Clients authenticate with per-user gateway keys (`Authorization: Bearer ar_…`, sha256 `agent-router-gw:` prefix). Model names route automatically (`gpt-*`→OpenAI, `claude-*`→Anthropic, `gemini-*`→Gemini); `x-provider-id` pins a provider. Codex OAuth (`OPENAI_CODEX`, ChatGPT subscription) only serves `POST /v1/responses` via the Codex backend (`chatgpt.com/backend-api/codex`); chat/embeddings need a platform `OPENAI` provider.
- **Key pools**: each provider holds many upstream keys with admin-set `tokenLimit`/`requestLimit` caps, `priority` ordering, and monthly `resetInDays` rollover. Secrets are AES-GCM encrypted (Secrets Store `PROVIDER_KEYS_ENCRYPTION_SECRET` for static keys, `CODEX_OAUTH_ENCRYPTION_SECRET` for Codex OAuth tokens) and never returned by the API (hint only).
