...
- `apps/web/vite.config.ts` proxies `/user` + `/api` + `/v1` + `/anthropic` + `/gemini` → `http://localhost:8787` in dev (`/api` covers the public Codex OAuth callback); `closeBundle` embeds `dist/index.html` into `apps/api/src/generated/spa-shell.ts` (`SPA_HTML`) on build.
...
Set `SITE_URL` to the exact public origin (e.g. `https://gw.example.com`) on custom-domain deployments — the Codex OAuth `redirect_uri` prefers it over the request origin, otherwise OpenAI rejects the callback with `redirect_uri_mismatch`.
