...
export default tseslint.config(
  ...
  // Layer 0: shared — zero @agent-router/* deps
  {
      ...
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@agent-router/*'],
              message: 'shared must not import from other @agent-router packages — it is a zero-dependency base layer',
            },
          ],
        },
      ],
  ...
  // Layer 0: backend-errors — zero @agent-router/* deps
  {
      ...
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@agent-router/*'],
              message: 'backend-errors must not import from other @agent-router packages — it is a zero-dependency base layer',
            },
          ],
        },
      ],
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@agent-router/backend-data', '@agent-router/backend-data/*'],
              message: 'backend-runtime must not import from backend-data (higher layer)',
            },
            {
              group: ['@agent-router/git-protocol', '@agent-router/git-protocol/*'],
              message: 'backend-runtime must not import from git-protocol (higher layer)',
            },
            {
              group: ['@agent-router/git-service', '@agent-router/git-service/*'],
              message: 'backend-runtime must not import from git-service (higher layer)',
            },
            {
              group: ['@agent-router/backend-services', '@agent-router/backend-services/*'],
              message: 'backend-runtime must not import from backend-services (higher layer)',
            },
            { group: ['@agent-router/api', '@agent-router/api/*'], message: 'backend-runtime must not import from apps/api' },
            { group: ['@agent-router/background', '@agent-router/background/*'], message: 'backend-runtime must not import from apps/background' },
          ],
        },
      ],
    },
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@agent-router/backend-runtime', '@agent-router/backend-runtime/*'],
              message: 'backend-data must not import from backend-runtime',
            },
            { group: ['@agent-router/git-protocol', '@agent-router/git-protocol/*'], message: 'backend-data must not import from git-protocol' },
            {
              group: ['@agent-router/git-service', '@agent-router/git-service/*'],
            ...
            { group: ['@agent-router/api', '@agent-router/api/*'], message: 'backend-data must not import from apps/api' },
            { group: ['@agent-router/background', '@agent-router/background/*'], message: 'backend-data must not import from apps/background' },
          ],
        },
      ],
    },
  ...
  // Layer 3: backend-services — cannot import apps
  {
    ...
    rules: {
      'no-restricted-imports': [
        ...
        {
          patterns: [
            { group: ['@agent-router/api', '@agent-router/api/*'], message: 'backend-services must not import from apps/api' },
            { group: ['@agent-router/background', '@agent-router/background/*'], message: 'backend-services must not import from apps/background' },
          ],
        },
      ],
    },
  },
  // Layer 5: apps/api — resolve state via backend-services composition, never DAOs directly.
  // Exception: apps/api/src/index.ts re-exports CronTasksWorker
  // from @agent-router/background for DO bindings; no other api→background value imports are used.
  {
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          patterns: [
            {
              group: ['@agent-router/backend-data/dao', '@agent-router/backend-data/dao/*'],
              message: 'apps/api must not import DAOs directly; use @agent-router/backend-services instead (type-only imports are allowed)',
  ...
  // Layer 5: apps/background — layers 0-3 allowed (not apps/api)
  {
    ...
    rules: {
      'no-restricted-imports': [
        'error',
        {
          patterns: [{ group: ['@agent-router/api', '@agent-router/api/*'], message: 'apps/background must not import from apps/api' }],
        },
      ],
    },
  },
