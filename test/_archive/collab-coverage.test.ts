...
import { CollaborationDAO, PullRequestDAO } from '@agent-router/backend-data/dao';
import { CollaborationService } from '@agent-router/backend-services/collab';
import { PullRequestService } from '@agent-router/backend-services/pull';
...
import { ReadModelService } from '@agent-router/background/ReadModelService';
...
describe('MergeService squash and rebase', () => {
  ...
  async function makeRepo(): Promise<{ dir: string; gitdir: string }> {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-collab-'));
    tmpDirs.push(dir);
...
describe('HistoryService blame and GitService wrappers', () => {
  ...
  async function makeBlameRepo(): Promise<{ gitdir: string; tip: string }> {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-blame-'));
    tmpDirs.push(dir);
    await git.init({ fs, dir, defaultBranch: 'main' });
  ...
  it('returns null for binary files', async () => {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-blame-bin-'));
    tmpDirs.push(dir);
    await git.init({ fs, dir, defaultBranch: 'main' });
