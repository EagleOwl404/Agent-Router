...
describe('DeployKeyService hardening', () => {
  ...
  it('hashKey is namespaced (differs from PAT hash)', async () => {
    const svc = new (await import('@agent-router/backend-services/auth')).TokenService({ DB: {} } as never);
    void svc;
    const deployHash = await DeployKeyService.hashKey('secret');
    const { CryptoUtil } = await import('@agent-router/shared/utils');
    const patHash = await CryptoUtil.sha256Hex('agent-router-pat:secret');
