...
import { buildFetchErrorResponse, parseFetchRequest, validateFetchRequestCounts, validateReceivePackCounts } from '@agent-router/git-protocol';
...
describe('partial-clone filters with explicit wants', () => {
  ...
  async function makeRepo(): Promise<{ gitdir: string; commitOid: string; treeOid: string; blobOid: string }> {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-filter-'));
    tmpDirs.push(dir);
