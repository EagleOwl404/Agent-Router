...
describe('MergeService true merge', () => {
  ...
  async function makeRepo(): Promise<{ dir: string; gitdir: string }> {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-merge-'));
    tmpDirs.push(dir);
  ...
  function mergerFor(gitdir: string): MergeService {
    return new MergeService(fs as never, gitdir, path.join(os.tmpdir(), 'agent-router-merge-wd'));
  }
