...
import { ConfigurationManager } from '@agent-router/backend-runtime/config/ConfigurationManager';
import { TokenService } from '@agent-router/backend-services/auth';
import type { D1Queryable } from '@agent-router/backend-data/utils';
import { IssueService } from '@agent-router/backend-services/issue/IssueService';
import { validateWebhookUrl, verifyDeliverySignature, signDelivery } from '@agent-router/backend-services/webhook/WebhookEvents';
import { normalizePublicGitUrl, getBasicCredentials, getBearerToken, validateFilterSpec } from '@agent-router/git-protocol';
import { RepoFullName } from '@agent-router/shared/utils/Identity';
...
describe('SSRF guard hardening', () => {
  it('rejects encoded, IPv6-private, and malformed hosts for webhooks', () => {
    ...
    expect(() => validateWebhookUrl('https://hooks.example.com/agent-router')).not.toThrow();
  });
