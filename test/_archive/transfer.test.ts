...
} from '@agent-router/git-protocol';
import { scanBytes, scanText } from '@agent-router/backend-services/security';
import { SecuritySettingsService } from '@agent-router/backend-services/security';
import { DeployKeyService } from '@agent-router/backend-services/deploykey';
import { ImportService } from '@agent-router/backend-services/transfer/ImportService';
import { MirrorService } from '@agent-router/backend-services/transfer/MirrorService';
...
describe('DeployKeyService', () => {
  ...
  it('mints one-time keys with distinct hash domain and authenticates per repo', async () => {
    ...
    const patHash = await (await import('@agent-router/backend-services/auth')).TokenService.hashToken(created.key);
