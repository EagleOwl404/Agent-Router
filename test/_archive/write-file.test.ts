...
describe('WriteService commitFile', () => {
  ...
  async function makeRepo(): Promise<{ svc: WriteService; dir: string; gitdir: string }> {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-write-'));
    tmpDirs.push(dir);
  ...
  async function makeEmptyRepo(): Promise<{ svc: WriteService; dir: string }> {
    const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'agent-router-write-empty-'));
    tmpDirs.push(dir);
    await git.init({ fs, dir, defaultBranch: 'main' });
